...
def test_summarize_one_variant(snapshot):
    ...
    output = summarize(eval_results=results, agents=agents, baseline=agent_1.id, evaluators=["FluencyEvaluator", "RelevanceEvaluator"], agent_base_url="https://ai-url/")

    snapshot.snapshot_dir = Path("tests", "snapshots", "summarize")
    snapshot.assert_match(output, f"one_variant.md")
...
def test_summarize_multiple_variants(snapshot):
    ...
    snapshot.snapshot_dir = Path("tests", "snapshots", "summarize")
    snapshot.assert_match(output, f"two_variants.md")
